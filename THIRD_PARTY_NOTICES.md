...
Role: comparative reverse-engineering reference for OutRun 2 Special Tours Deluxe/Lindbergh FFB event semantics and timing. The standalone FFB branch records observations from `OutRun2Real.cpp` and `OutRun2Fake.cpp`, but does not copy Lindbergh absolute addresses or plugin force-output code into the C2C runtime.

License: GNU GPL version 3. No FFBArcadePlugin implementation code is incorporated by the witness/logging change; this notice attributes the public reference used by `docs/reverse/LINDBERGH_FFB_MAP.md`.
...
역할: OutRun 2 Special Tours Deluxe/Lindbergh의 FFB 이벤트 의미와 타이밍을 비교 역분석하기 위한 공개 참고 자료입니다. 독립 FFB 브랜치는 `OutRun2Real.cpp`, `OutRun2Fake.cpp`의 관찰 결과만 기록하며 Lindbergh 절대주소나 플러그인의 출력 구현을 C2C 런타임에 복사하지 않습니다.

라이선스: GNU GPL version 3. 이번 witness/로그 변경에는 FFBArcadePlugin 구현 코드를 포함하지 않으며, `docs/reverse/LINDBERGH_FFB_MAP.md` 작성에 사용한 공개 참고 자료의 출처를 명시합니다.
