...
$meta = Join-Path $root 'SMALL12_VARIANT.txt'
if (-not (Test-Path $meta)) { throw 'SMALL12_VARIANT.txt missing.' }
...
$logRoot = Join-Path $root 'DX9EX_SMALL12_LOGS'
...
Copy-Item $meta (Join-Path $session 'SMALL12_VARIANT.txt')

Write-Host "DX9Ex Small12 test: $variant"
Write-Host "Check loading, world geometry, HUD 6th/6 size/position, white rank/YES-NO, vehicle rank, recenter, and stutter."
...
$zip=Join-Path $logRoot ("DX9EX_SMALL12_LOG_{0}_{1}.zip" -f $variant,$stamp)
