...
Invoke-Checked cmake '-S' '.' '-B' $gameBuild '-G' 'Visual Studio 17 2022' '-A' 'Win32' '-DOUTRUN_VR_SAFE_DRAW_COMPARE=OFF' '-DOUTRUN_VR_R26_HUD_COMPARE=ON' '-DOUTRUN_VR_C1_COMPARE=OFF' '-DOUTRUN_VR_C2_COMPARE=OFF'
...
Set-Content (Join-Path $backendDir 'VARIANT_ID.txt') 'R51_VIEW2_CAR_R26_R30' -Encoding ascii
Set-Content (Join-Path $backendDir 'CMAKE_FLAGS.txt') '-DOUTRUN_VR_SAFE_DRAW_COMPARE=OFF -DOUTRUN_VR_R26_HUD_COMPARE=ON -DOUTRUN_VR_C1_COMPARE=OFF -DOUTRUN_VR_C2_COMPARE=OFF' -Encoding ascii
...
$buildInputs = [ordered]@{
    ...
    VariantId = 'R51_VIEW2_CAR_R26_R30'
