...
req(math, 'PrimaryRoughRoadSurfaceMask = 0x00100000u', 'primary rough-road material uses proven mask 0x100000')
...
req(build, 'floralComfort ? FloralVillageRoughPavingScale : 1.0f', 'only Floral gets the tested comfort attenuation')
req(build, 'PrimarySnowIceSurfaceMask', 'snow attenuation/latch logic uses the proven material mask')
req(build, 'primarySnowAsphaltOnly', 'snow-to-primary-asphalt transition cannot be latched as a curb')
req(build, 'WheelFFB ROAD EDGE: floralRoughPaving={}', 'Floral entry/exit diagnostic is edge-driven')
req(ffb, 'is_proven_primary_rough_road_contact(', 'Arcade rough classifier excludes proven primary rough roads')
req(stage_map, 'Floral Village (27)', 'stage map records the proven Floral Village primary rough-road material')
req(stage_map, '510..533', 'stage map records the proven Floral Village rough-road section range')
...
req(ffb, 'authoritativeCanRetrigger', 'authoritative collisions can retrigger after active output window')
req(ffb, 'crashArcadeStrength_', 'Arcade collision latches pre-impact strength')
req(ffb, 'speedFallbackCooldown_', 'speed-drop heuristic has independent long cooldown')
req(ffb, 'reversalBuildAssistFrames_ = 4;', 'counter-torque rebuild gets bounded post-zero assist')
req(math, 'software_road_tactile_frequency', 'software road tactile carrier has smoother dedicated frequency')
req(math, 'software_slip_tactile_frequency', 'software slip carrier remains separate from road')
req(ffb, 'if (!roadTextureEffect_)', 'road hardware/fallback transport is channel-independent')
req(ffb, 'if (!tireSlipEffect_)', 'slip hardware/fallback transport is channel-independent')
