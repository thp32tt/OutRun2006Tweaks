...
# 1. Finish the hybrid input migration and restore the legacy fallback setting.
...
# 2. d-b-c-e toolkit v0.8.0 lesson: gain/invert before saturation and slew.
ffb = rep(
    ffb,
...
'''            float roadAmp =\n                textureRoughness * roadSpeedGate *\n                static_cast<float>(Settings::WheelFFBRoadTexture) * outputStrength;\n''',
    'master gain on hardware/fallback road detail')
...
'''                splashAmp_ =\n                    (roughness - 0.7f) * speedNorm * 0.75f * roadTextureScale * outputStrength;\n''',
    'master gain on water/splash detail')
...
'''            // Gain and invert now happen before tanh/slew. This mirrors the\n            // d-b-c-e toolkit v0.8.0 ordering and prevents the slew limiter from\n            // comparing values expressed in two different scales/signs.\n            float total = (structural + events) * outputStrength;\n            if (Settings::WheelFFBInvertForce)\n''',
    'gain before soft saturation and slew')

gain_block = '''effect.dwGain = static_cast<DWORD>(\n                std::clamp(static_cast<float>(Settings::WheelFFBGlobalStrength), 0.0f, 1.0f) *\n                static_cast<float>(DI_FFNOMINALMAX));'''
gain_count = ffb.count(gain_block)
if gain_count < 3:
    raise SystemExit(f'full DirectInput effect gain: expected at least 3 create paths, found {gain_count}')
ffb = ffb.replace(gain_block, 'effect.dwGain = DI_FFNOMINALMAX;')
print(f'community patch: full DirectInput device gain on {gain_count} effect create paths')

ffb = replace_between(
    ffb,
    '        DWORD configured_effect_gain() const\n',
    '        void apply_live_effect_gain()\n',
'''        DWORD configured_effect_gain() const\n        {\n            // User strength is part of the software force model. DirectInput\n            // dwGain stays at its device-side nominal ceiling.\n            return DI_FFNOMINALMAX;\n        }\n\n''',
    'keep DirectInput dwGain nominal after v0.8-style gain move')


# 3. Actuator-axis detection, POLAR fallback, and sibling-interface recovery.
if '#include <vector>' not in ffb:
    ffb = rep(ffb, '#include <string>\n', '#include <string>\n#include <vector>\n', 'vector support for actuator axes')
...
'''            const std::string wantedGuid =\n                lower_copy(Settings::WheelFFBDeviceGuid.get().c_str());\n\n            if (ctx->self && GetTickCount() < ctx->self->failedInterfaceUntil_ &&\n                directinput_guid_key(instance->guidInstance) == ctx->self->failedInterfaceGuid_)\n            {\n                spdlog::warn(\n                    "WheelFFB: temporarily skipping rejected FFB interface '{}' [{}] to try a sibling",\n                    instance->tszProductName, directinput_guid_key(instance->guidInstance));\n                return DIENUM_CONTINUE;\n            }\n''',
    'skip recently rejected duplicate FFB interface')
...
'''
ffb = rep(ffb, '        bool initialize()\n        {\n', helpers + '        bool initialize()\n        {\n', 'actuator-axis and sibling helpers')
...
'''            actuatorAxes_.clear();\n            const HRESULT axisEnumHr = device_->EnumObjects(\n                enum_actuator_axis_callback, this, DIDFT_AXIS);\n            if (FAILED(axisEnumHr) || actuatorAxes_.empty())\n            {\n                actuatorAxes_.clear();\n                actuatorAxes_.push_back(DIJOFS_X);\n                spdlog::warn(\n                    "WheelFFB: no explicit DIDFT_FFACTUATOR axis reported; using DIJOFS_X fallback");\n            }\n            else\n            {\n                spdlog::info(\n                    "WheelFFB: detected {} force actuator axis/axes; primary offset={}",\n                    actuatorAxes_.size(), (unsigned)actuatorAxes_.front());\n            }\n\n            if (!create_constant_effect())\n            {\n''',
    'enumerate FFB actuator axes before creating effects')
...
constant_create = r'''        bool create_constant_effect()
        {
            ...
            if (actuatorAxes_.size() > 1)
            {
                ...
                if (SUCCEEDED(hr) && constantEffect_)
                {
                    ...
                    spdlog::info(
                        "WheelFFB: ConstantForce created with 2-axis POLAR actuator encoding");
                    return true;
                }
...
ffb = replace_between(
    ...
    constant_create,
    'actuator-aware POLAR/CARTESIAN ConstantForce creation')

fixed_axis = 'DWORD axes[1] = { DIJOFS_X };'
fixed_count = ffb.count(fixed_axis)
if fixed_count < 3:
    raise SystemExit(f'primary actuator axis replacement: expected >=3, found {fixed_count}')
ffb = ffb.replace(fixed_axis, 'DWORD axes[1] = { primary_actuator_axis() };')
print(f'community patch: primary actuator axis on {fixed_count} condition/periodic create paths')
...
'''        IDirectInputEffect* tireSlipEffect_ = nullptr;\n        std::vector<DWORD> actuatorAxes_;\n        bool constantEffectPolar_ = false;\n\n        GUID selectedGuid_{};\n''',
    'actuator/constant encoding state')

ffb = rep(
    ffb,
'''        DWORD retryAfter_ = 0;\n''',
'''        DWORD retryAfter_ = 0;\n        std::string failedInterfaceGuid_;\n        DWORD failedInterfaceUntil_ = 0;\n''',
    'temporary sibling-interface skip state')


# 4. Safe 20% left/right tests in F11.
ffb = rep(
    ffb,
'''        void check_watchdog()\n        {\n''',
'''        void request_direction_test(int direction)\n        {\n            if (direction == 0)\n            {\n                manualTestFrames_ = 0;\n                if (initialized_ && !panicStopped_)\n                    set_constant_force(0);\n                return;\n            }\n            manualTestDirection_ = direction < 0 ? -1 : 1;\n            manualTestFrames_ = 18;\n            spdlog::info(\n                "WheelFFB: queued safe {} direction test at fixed 20% output",\n                manualTestDirection_ < 0 ? "left" : "right");\n        }\n\n        void check_watchdog()\n        {\n''',
    'public safe-direction test request')
...
'''
    print('community patch: exported safe-direction test request')


# 5. Versioned F11 baseline presets.
uis = rep(
    uis,
...
'''    extern Setting<float> WheelFFBSteeringWeight;\n    extern Setting<float> WheelFFBGripLoss;\n    extern Setting<float> WheelFFBLowSpeedSpring;\n    extern Setting<float> WheelFFBSpringLoadBoost;\n''',
    'declare simulation preset controls')

settings_close = '''    extern Setting<bool> WheelFFBDebugLog;\n}\n'''
if 'void WheelFFB_RequestDirectionTest(int direction);' not in uis:
    uis = rep(
        uis,
        settings_close,
        settings_close + '\nvoid WheelFFB_RequestDirectionTest(int direction);\n',
        'declare safe-direction test API in F11 UI')
...
uis = rep(uis, ui_anchor, ui_new, 'versioned FFB presets in F11')
...
checks = {
    'src/input_manager.cpp': ['Setting<bool> WheelInputCompatibility', 'multi-device'],
    'src/hooks_wheel_ffb.cpp': [
        'Range<float>{ 0.0f, 1.5f }',
        'Gain and invert now happen before tanh/slew',
        'DIDFT_FFACTUATOR',
        '2-axis POLAR actuator encoding',
        'one-axis CARTESIAN fallback',
...
print('Applied d-b-c-e v0.8.0 + hyp36rmax/Discord FFB lessons to the hybrid wheel build')
