...
$meta = Join-Path $root 'WHITE_VARIANT.txt'
if (-not (Test-Path $meta)) { throw 'WHITE_VARIANT.txt missing.' }
...
if (-not (Test-Path $game)) { throw 'OR2006C2C.EXE not found. Extract this ZIP into the game folder.' }
...
$logRoot = Join-Path $root 'DX9EX_WHITE_LOGS'
...
Copy-Item $meta (Join-Path $session 'WHITE_VARIANT.txt')

Write-Host "DX9Ex white-overlay test: $variant"
Write-Host "Check white rank/score, YES-NO, 6th/6, HUD scale/lock, car-attached rank and stutter."
...
$zip = Join-Path $logRoot ("DX9EX_WHITE_LOG_{0}_{1}.zip" -f $variant,$stamp)
