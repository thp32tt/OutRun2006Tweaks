...
$meta = Join-Path $root 'R13_VARIANT.txt'
if (-not (Test-Path $meta)) { throw 'R13_VARIANT.txt missing.' }
...
if (-not (Test-Path $game)) { throw 'OR2006C2C.EXE not found. Extract ZIP into game folder.' }
...
$logRoot = Join-Path $root 'DX9EX_R13_LOGS'
...
Copy-Item $meta (Join-Path $session 'R13_VARIANT.txt')

Write-Host "DX9Ex R13 flat-overlay test: $variant"
Write-Host "Check stage-loading visibility, white rank/YES-NO, normal HUD, car rank, recenter and stutter."
...
$zip=Join-Path $logRoot ("DX9EX_R13_LOG_{0}_{1}.zip" -f $variant,$stamp)
