...
namespace
{
    ...
    std::array<std::uint32_t, OutRunVR::RenderFrameRingSize>
        R48DirectTouchedFrame{};
    std::array<std::uint32_t, OutRunVR::RenderFrameRingSize>
        R48DirectTouchedGeneration{};
        ...
        std::uint32_t newWatermark) noexcept
    {
        ...
        OutRunVrR32DirectSubmit::ObserveGeneration(currentGeneration);
        ...
        for (std::size_t i = 0; i < count; ++i)
        {
            ...
            const bool touched =
                R48DirectTouchedFrame[slot] == frame.frameId &&
                R48DirectTouchedGeneration[slot] == generation;
            if (touched ||
                ...
                if (presentation == OutRunVR::PresentationGameplay)
                {
                    ...
                    if (directTransportOnly)
                    {
                        ...
                        if (renderFrames.ReadHistory(history, historyCount))
                        {
                            ...
                            if (foundDirect)
                            {
                                OutRunVrR32DirectSubmit::ObserveGeneration(
                                    currentGeneration);
                                for (std::size_t i = 0; i < historyCount; ++i)
                                {
                                    const auto& frame = history[i];
                                    ...
                                    const bool touched =
                                        R48DirectTouchedFrame[slot] ==
                                            frame.frameId &&
                                        R48DirectTouchedGeneration[slot] ==
                                            generation;
                                    if (touched ||
