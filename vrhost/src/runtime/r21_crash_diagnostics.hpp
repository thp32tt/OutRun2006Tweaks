...
// R22 crash evidence for the x64 OpenXR host.
// Produces a unique text fault record plus a minidump beside the host executable.
// This is deliberately diagnostic only; it does not attempt recovery from an
// access violation or alter the OpenXR/D3D11 runtime state. An out-of-process
// dumper is still preferable for a severely corrupted process, but this handler
// records MiniDumpWriteDump success/failure so in-process dump reliability is
...
{
    inline constexpr const char* BuildId = "R22-crash-dump-20260916";
    ...
    inline void WriteFaultText(EXCEPTION_POINTERS* ep,
        const wchar_t* baseName, const DumpResult& dump) noexcept
    {
        wchar_t fileName[160]{};
    ...
    inline LONG WINAPI UnhandledExceptionFilter(EXCEPTION_POINTERS* ep) noexcept
    {
        ...
        WriteFaultText(ep, baseName, dump);
